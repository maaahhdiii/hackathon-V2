    ...
    <div class="grid">
        ...
        <div class="card">
            ...
            <select id="vuln">
                ...
                <option value="priv_esc">priv_esc</option>
            </select>
        ...
        <div class="card">
            <h3>DB Endpoints</h3>
            <input id="search" value="a" />
            <button onclick="callApi('POST','/query',{search:val('search')})">Search Users</button>
            <input id="uid" type="number" value="1" />
            <button onclick="callApi('GET','/user/'+num('uid'))">Get User</button>
            <input id="requester" type="number" value="2" />
            <button onclick="callApi('POST','/user/'+num('uid')+'/promote',{requester_id:num('requester')})">Promote User</button>
            <button onclick="callApi('GET','/tables')">List Tables</button>
        </div>
    </div>
    ...
    return jsonify(
        {
            ...
            "message": "Use the vulnerable DB endpoints directly instead of /attack.",
        }
    ), 410
